        ...
        $config = array(
               ...
               if ($this->form_validation->run() == FALSE)
                ...
                if(empty($teacher_email[0])){
                  //$data = json_decode ("{}");
                  $data;
                  $done = array("status"=>"0","msg"=>"عفوا لا توجد بيانات لهذا المدرس","data"=>null);
                  $this->set_response($done, REST_Controller::HTTP_OK);
                }
                else{
                  $data = array(
                  ...
                  $update =$this->MainModel->updatedata('teacher','teacher_id',$teacher_email[0]['teacher_email'],$data);
                  if($update){
                    unset($teacher_email[0]['teacher_password']);
                    unset($teacher_email[0]['teacher_token_web']);
                    unset($teacher_email[0]['teacher_enterdate']);
                    $done = array("status"=>"1","msg"=>"تم تغيير كلمة المرور بنجاح","data"=>array("user"=>$teacher_email[0]));
                     $this->set_response($done, REST_Controller::HTTP_OK);
                  }
                  else{
