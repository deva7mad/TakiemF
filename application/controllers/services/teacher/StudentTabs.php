...
class StudentTabs extends REST_Controller
{
  ...
  public function index_get()
    {
      ...
      $data = array(
        ...
        'student_main_id' => $mainID,
        'student_sub_tab' => $subID
      );
      //$mainTabs = $this->MainModel->getwherearray('group_tab_student',$data,'');
	  $mainTabs = $this->MainModel->getwherearray('students_tabs',$data,'');
      if(!empty($mainTabs)){


       $this->set_response(array("status"=>"1","msg"=>"جميع الطلاب","data"=>array('groups'=>$mainTabs)), REST_Controller::HTTP_OK);

     }else{
       $done = array("status"=>"0","msg"=>"لا يوجد طلاب","data"=>null);
